...
from scripts.run_formal_benchmarks import _payload_sha256
from scripts.summarize_formal_records import summary_fieldnames
...
from test_formal_benchmark_schema import RESULT_HASH, ROWS, valid_request, valid_setup
...
def _make_bundle(tmp_path: Path) -> tuple[Path, Path, Path, Path, Path]:
    ...
    git_commit = "a" * 40
    environment = {
        "git": {"commit": git_commit, "branch": "student/test", "dirty": False, "dirty_paths": []},
...
def _finalize(tmp_path: Path) -> Path:
    from scripts.formal_evidence_bundle import finalize_bundle
...
def test_finalize_and_audit_complete_formal_bundle(tmp_path: Path) -> None:
    from scripts.formal_evidence_bundle import audit_bundle
...
def test_finalize_rejects_oracle_hash_not_derived_from_exact_rows(
    ...
    environment_path = root / "environment.json"
    environment = json.loads(environment_path.read_text(encoding="utf-8"))
    environment["formal_runner"]["matrix_sha256"] = _sha256(matrix_path)
    environment["formal_runner"]["matrix_payload"] = matrix
    environment["formal_runner"]["matrix_payload_sha256"] = _payload_sha256(matrix)
    _write_json(environment_path, environment)

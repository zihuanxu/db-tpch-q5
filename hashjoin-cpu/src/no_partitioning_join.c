...
static uint64_t
bucket_buffer_chain_bytes(const bucket_buffer_t * buf)
{
    uint64_t bytes = 0;

    while(buf) {
        bytes += sizeof(bucket_buffer_t);
        buf = buf->next;
    }
...
static uint64_t
npo_primary_bucket_bytes(const hashtable_t * ht)
{
    return (uint64_t) ht->num_buckets * sizeof(bucket_t);
}

static uint64_t
npo_linked_overflow_bucket_bytes(const hashtable_t * ht)
{
    uint64_t bytes = 0;
    int32_t i;

    for(i = 0; i < ht->num_buckets; i++) {
        const bucket_t * bucket = ht->buckets[i].next;
        while(bucket) {
            bytes += sizeof(bucket_t);
            bucket = bucket->next;
        }
    }
...
static void
npo_free_linked_overflow_buckets(hashtable_t * ht)
{
    int32_t i;

    for(i = 0; i < ht->num_buckets; i++) {
        bucket_t * bucket = ht->buckets[i].next;
...
NPO_st(relation_t *relR, relation_t *relS, int nthreads)
{
    ...
    int64_t result = 0;
    result_t * joinresult;
    uint64_t primary_hash_table_bytes;
    uint64_t overflow_hash_table_bytes;
    ...
    memset(joinresult, 0, sizeof(result_t));
#ifdef JOIN_RESULT_MATERIALIZE
    ...
    joinresult->has_npo_stats = 1;
    primary_hash_table_bytes = npo_primary_bucket_bytes(ht);
    overflow_hash_table_bytes = npo_linked_overflow_bucket_bytes(ht);
    joinresult->npo_primary_hash_table_bytes = primary_hash_table_bytes;
    joinresult->npo_overflow_hash_table_bytes = overflow_hash_table_bytes;
    joinresult->npo_hash_table_bytes =
        primary_hash_table_bytes + overflow_hash_table_bytes;
    joinresult->total_extra_space_bytes =
        joinresult->npo_hash_table_bytes + sizeof(result_t);

    npo_free_linked_overflow_buckets(ht);
...
npo_thread(void * param)
{
    ...
    args->overflowbuf_bytes = bucket_buffer_chain_bytes(overflowbuf);
...
NPO(relation_t *relR, relation_t *relS, int nthreads)
{
    ...
    uint64_t primary_hash_table_bytes;
    uint64_t overflow_hash_table_bytes = 0;
    joinresult = (result_t *) malloc(sizeof(result_t));
    memset(joinresult, 0, sizeof(result_t));
    ...
    for(i = 0; i < nthreads; i++){
        ...
        overflow_hash_table_bytes += args[i].overflowbuf_bytes;
    }
    joinresult->totalresults = result;

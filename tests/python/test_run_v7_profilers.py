...
from scripts import run_v7_profilers
...
def test_dry_run_prints_exact_commands_and_ten_bound_identities(
    ...
    monkeypatch.setattr(
        run_v7_profilers, "_audit_evidence_bundle", _successful_evidence_audit
    )
    monkeypatch.setattr(
        run_v7_profilers,
        "_run_command",
    ...
    assert run_v7_profilers.main([*_argv(tmp_path, inputs), "--dry-run"]) == 0
...
def test_success_stages_inputs_binds_observed_auto_ratio_and_audits(
    ...
    monkeypatch.setattr(run_v7_profilers, "_run_command", _fake_collectors(calls))
    monkeypatch.setattr(
        run_v7_profilers,
        "_finalize_bundle",
    ...
    assert run_v7_profilers.main(_argv(tmp_path, inputs)) == 0
...
def test_ncu_counter_permission_failure_is_the_only_unavailable_downgrade(
    ...
    monkeypatch.setattr(run_v7_profilers, "_finalize_bundle", lambda _root: 0)
    monkeypatch.setattr(run_v7_profilers, "_audit_profiler_bundle", lambda _root: 0)
...
def test_other_collector_failure_stops_and_preserves_failure_metadata(
    ...
    assert run_v7_profilers.main(_argv(tmp_path, inputs)) == 1
...
def test_evidence_must_audit_before_dry_run_or_output_creation(
    ...
    assert run_v7_profilers.main([*_argv(tmp_path, inputs), "--dry-run"]) == 1
    assert not (tmp_path / "profiler-bundle").exists()

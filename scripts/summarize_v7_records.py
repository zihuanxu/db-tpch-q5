...
def _group_key(record: V7BenchmarkRecord) -> tuple[object, ...]:
    return tuple(getattr(record, field) for field in GROUP_FIELDS)


def _is_profiler_row(record: V7BenchmarkRecord) -> bool:
...
def summarize_records(
    records: Sequence[V7BenchmarkRecord], setups: Sequence[V7SetupRecord]
    ...
    setup_groups: dict[str, list[V7SetupRecord]] = defaultdict(list)
    for setup in setups:
        setup_groups[setup.config_id].append(setup)
    record_groups: dict[tuple[object, ...], list[V7BenchmarkRecord]] = defaultdict(list)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize V7 resident request records")

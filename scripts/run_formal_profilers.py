#!/usr/bin/env python3
"""Run and freeze the canonical Formal SF1/SF10 profiler matrix."""
...
try:
    from scripts.formal_evidence_bundle import audit_bundle as _audit_evidence_bundle
    from scripts.formal_profiler_bundle import (
    ...
    from formal_evidence_bundle import audit_bundle as _audit_evidence_bundle
    from formal_profiler_bundle import (  # type: ignore[no-redef]
        HYBRID_FIXED_CPU_RATIO,
        _recognized_ncu_unavailable,
        audit as _profiler_audit,
        finalize as _profiler_finalize,
...
def _public_plan(
    ...
    bundle_script = Path(__file__).resolve().with_name("formal_profiler_bundle.py")
    return {
        "dry_run": dry_run,
        "output_dir": str(root),
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Collect and audit the canonical Formal SF1/SF10 profiler bundle"
    )

 ...
 *  - PRVJ:   Parallel Radix Vector Join
 ...
 * options: 
 * @verbatim
      Join algorithm selection, algorithms : RJ, PRO, PRVJ, PRH, PRHO, NPO, NPO_st
...
#include "vector_join.h"          /* vector join: VJ */
#include "starjoin.h"             /* starjoin: npo, pro, vj */
#include "generator.h"            /* create_relation_xk */

#include "constants.h"     /* DEFAULT_R_SEED, DEFAULT_R_SEED */
#include "prj_params.h"    /* NUM_PASSES, NUM_RADIX_BITS */
...
struct param_t {
    ...
    int vj_hugepage;
    starjoin_mode_t starjoin_mode;
    uint32_t sf;
    uint32_t starjoin_prefetch_distance;
    int starjoin_pro_order;
...
static void
print_result_statistics(const char * algo_name, const result_t * results);
...
main(int argc, char ** argv)
{
    ...
    relation_t relS;
    result_t * results;
    starjoin_input_t star_input;
    starjoin_result_t star_result;
    ...
    cmd_params.payload_width = 4;
    cmd_params.vj_hugepage = 0;
    cmd_params.starjoin_mode = STARJOIN_NONE;
    cmd_params.sf = 100;
    cmd_params.starjoin_prefetch_distance = 0;
    cmd_params.starjoin_pro_order = 0;
    ...
    vector_join_set_payload_width(cmd_params.payload_width);
    vector_join_set_hugepage(cmd_params.vj_hugepage);

    if(cmd_params.starjoin_mode != STARJOIN_NONE) {
        starjoin_params_t star_params;
        star_params.mode = cmd_params.starjoin_mode;
        star_params.nthreads = cmd_params.nthreads;
        star_params.sf = cmd_params.sf;
        star_params.payload_width = cmd_params.payload_width;
        star_params.basic_numa = cmd_params.basic_numa;
        star_params.prefetch_distance = cmd_params.starjoin_prefetch_distance;
        star_params.pro_order = cmd_params.starjoin_pro_order;
        ...
        fprintf(stderr,
                "[INFO ] Creating starjoin dataset sf=%u mode=%s nthreads=%u\n",
                cmd_params.sf,
                starjoin_mode_name(cmd_params.starjoin_mode),
                cmd_params.nthreads);
        starjoin_generate(&star_input, &star_params);

        fprintf(stderr, "[INFO ] Running starjoin mode %s ...\n",
                starjoin_mode_name(cmd_params.starjoin_mode));
        if(starjoin_run(&star_result, &star_input, &star_params) != 0) {
            fprintf(stderr, "[ERROR] starjoin execution failed\n");
            starjoin_free(&star_input);
            return EXIT_FAILURE;
        }

        fprintf(stderr, "[INFO ] Starjoin matches = %llu aggregate = %lld\n",
                (unsigned long long) star_result.match_count,
                (long long) star_result.aggregate_sum);
        fprintf(stdout,
                "STARJOIN_STATS_CSV_HEADER,mode,sf,nthreads,matches,aggregate_sum,"
                "input_bytes,aux_bytes,total_bytes\n");
        fprintf(stdout, "STARJOIN_STATS_CSV,%s,%u,%u,%llu,%lld,%llu,%llu,%llu\n",
                starjoin_mode_name(cmd_params.starjoin_mode),
                cmd_params.sf,
                cmd_params.nthreads,
                (unsigned long long) star_result.match_count,
                (long long) star_result.aggregate_sum,
                (unsigned long long) star_result.input_bytes,
                (unsigned long long) star_result.aux_bytes,
                (unsigned long long) star_result.total_bytes);
        starjoin_free(&star_input);
        return 0;
    }
    ...
    print_result_statistics(cmd_params.algo->name, results);
...
print_result_statistics(const char * algo_name, const result_t * results)
{
    ...
    if(strcmp(algo_name, "VJ") == 0 && results->has_vector_join_stats) {
        fprintf(stderr,
                "[INFO ] VJ stats: payload_width_bytes=%llu, "
                "vector_index_bytes=%llu, vector_total_bytes=%llu, "
                "total_extra_space_bytes=%llu\n",
                (unsigned long long) results->payload_width_bytes,
                (unsigned long long) results->vector_index_bytes,
                (unsigned long long) results->vector_total_bytes,
                (unsigned long long) results->total_extra_space_bytes);

        fprintf(stdout,
                "VJ_STATS_CSV_HEADER,payload_width_bytes,vector_index_bytes,"
                "vector_total_bytes,total_extra_space_bytes\n");
        fprintf(stdout, "VJ_STATS_CSV,%llu,%llu,%llu,%llu\n",
                (unsigned long long) results->payload_width_bytes,
                (unsigned long long) results->vector_index_bytes,
                (unsigned long long) results->vector_total_bytes,
                (unsigned long long) results->total_extra_space_bytes);
    }
    else if(strcmp(algo_name, "PRVJ") == 0 && results->has_prvj_stats) {
        fprintf(stderr,
                "[INFO ] PRVJ stats: payload_width_bytes=%llu, "
                "partition_vector_total_bytes=%llu, partition_count=%llu, "
                "radix_bits=%u, num_passes=%u\n",
                (unsigned long long) results->payload_width_bytes,
                (unsigned long long) results->partition_vector_total_bytes,
                (unsigned long long) results->partition_count,
                results->radix_bits,
                results->num_passes);

        fprintf(stdout,
                "PRVJ_STATS_CSV_HEADER,payload_width_bytes,"
                "partition_vector_total_bytes,partition_count,"
                "radix_bits,num_passes\n");
        fprintf(stdout, "PRVJ_STATS_CSV,%llu,%llu,%llu,%u,%u\n",
                (unsigned long long) results->payload_width_bytes,
                (unsigned long long) results->partition_vector_total_bytes,
                (unsigned long long) results->partition_count,
                results->radix_bits,
    ...
    else if((strcmp(algo_name, "NPO") == 0 ||
             strcmp(algo_name, "NPO_st") == 0) &&
            results->has_npo_stats) {
        fprintf(stderr,
                "[INFO ] NPO stats: hash_table_bytes=%llu, "
                "primary_bucket_bytes=%llu, overflow_bucket_bytes=%llu, "
                "total_extra_space_bytes=%llu\n",
                (unsigned long long) results->npo_hash_table_bytes,
                (unsigned long long) results->npo_primary_hash_table_bytes,
                (unsigned long long) results->npo_overflow_hash_table_bytes,
                (unsigned long long) results->total_extra_space_bytes);

        fprintf(stdout,
                "NPO_STATS_CSV_HEADER,hash_table_bytes,total_extra_space_bytes,"
                "primary_bucket_bytes,overflow_bucket_bytes\n");
        fprintf(stdout, "NPO_STATS_CSV,%llu,%llu,%llu,%llu\n",
                (unsigned long long) results->npo_hash_table_bytes,
                (unsigned long long) results->total_extra_space_bytes,
                (unsigned long long) results->npo_primary_hash_table_bytes,
                (unsigned long long) results->npo_overflow_hash_table_bytes);
    }
    else if((strcmp(algo_name, "sort-merge") == 0 ||
             strcmp(algo_name, "SORTMERGE") == 0) &&
            results->has_sortmerge_stats) {
        fprintf(stderr,
                "[INFO ] sort-merge stats: sortmerge_buffer_bytes=%llu, "
                "total_extra_space_bytes=%llu\n",
                (unsigned long long) results->sortmerge_buffer_bytes,
                (unsigned long long) results->total_extra_space_bytes);

        fprintf(stdout,
                "SORTMERGE_STATS_CSV_HEADER,sortmerge_buffer_bytes,total_extra_space_bytes\n");
        fprintf(stdout, "SORTMERGE_STATS_CSV,%llu,%llu\n",
                (unsigned long long) results->sortmerge_buffer_bytes,
                (unsigned long long) results->total_extra_space_bytes);
    }
}
...
print_help(char * progname)
{
    ...
    printf("\
    Join algorithm selection, algorithms : RJ, PRO, PRVJ, PRH, PRHO, NPO, NPO_st, VJ, sort-merge\n\
       -a --algo=<name>    Run the hash join algorithm named <name> [PRO]     \n\
                                                                              \n\
    Other join configuration options, with default values in [] :             \n\
       -n --nthreads=<N>  Number of threads to use <N> [2]                    \n\
       ...
       --payload-width=<W> VJ/PRVJ vector payload width, W in {1,2,4} bytes [4]\n\
       --vj-hugepage      Advise VJ vector allocations with MADV_HUGEPAGE      \n\
       --starjoin=<M>      Run starjoin mode M in {npo,pro,vj}                \n\
       --sf=<N>            Scale factor for starjoin dataset [100]             \n\
       --starjoin-prefetch-distance=<N> Prefetch distance for npo/vj starjoin [0]\n\
       --starjoin-pro-order=<N> Pro starjoin materialization order: 0=L-PS first, 1=L-O first [0]\n\
       --non-unique       Use non-unique (duplicated) keys in input relations \n\
       --full-range       Spread keys in relns. in full 32-bit integer range  \n\
       --basic-numa       Numa-localize relations to threads (Experimental)   \n\
                                                                              \n\
...
parse_args(int argc, char ** argv, param_t * cmd_params) 
{
    ...
    int c, i, found;
    enum {
        OPT_PAYLOAD_WIDTH = 1000,
        OPT_STARJOIN,
        OPT_SF,
        OPT_STARJOIN_PREFETCH_DISTANCE,
        OPT_STARJOIN_PRO_ORDER,
        OPT_VJ_HUGEPAGE
    };
    /* Flag set by ‘--verbose’. */
    ...
    while(1) {
        static struct option long_options[] =
            {
                ...
                {"payload-width", required_argument, 0, OPT_PAYLOAD_WIDTH},
                {"vj-hugepage", no_argument, 0, OPT_VJ_HUGEPAGE},
                {"starjoin", required_argument, 0, OPT_STARJOIN},
                {"sf", required_argument, 0, OPT_SF},
                {"starjoin-prefetch-distance", required_argument, 0, OPT_STARJOIN_PREFETCH_DISTANCE},
                {"starjoin-pro-order", required_argument, 0, OPT_STARJOIN_PRO_ORDER},
                {0, 0, 0, 0}
            };
        ...
        switch (c)
        {
          ...
          case OPT_PAYLOAD_WIDTH:
              cmd_params->payload_width = atoi(optarg);
              if(cmd_params->payload_width != 1 &&
                 cmd_params->payload_width != 2 &&
                 cmd_params->payload_width != 4) {
                  printf("[ERROR] --payload-width must be one of 1, 2, or 4.\n");
                  print_help(argv[0]);
                  exit(EXIT_FAILURE);
              }
              break;

          case OPT_VJ_HUGEPAGE:
              cmd_params->vj_hugepage = 1;
              break;

          case OPT_STARJOIN:
              if(strcmp(optarg, "npo") == 0) {
                  cmd_params->starjoin_mode = STARJOIN_NPO;
              }
              else if(strcmp(optarg, "pro") == 0) {
                  cmd_params->starjoin_mode = STARJOIN_PRO;
              }
              else if(strcmp(optarg, "vj") == 0) {
                  cmd_params->starjoin_mode = STARJOIN_VJ;
              }
              else {
                  printf("[ERROR] --starjoin must be one of npo, pro, or vj.\n");
                  print_help(argv[0]);
                  exit(EXIT_FAILURE);
              }
              break;
          ...
          case OPT_SF:
              cmd_params->sf = (uint32_t) atoi(optarg);
              if(cmd_params->sf == 0) {
                  printf("[ERROR] --sf must be positive.\n");
                  print_help(argv[0]);
                  exit(EXIT_FAILURE);
              }
              break;

          case OPT_STARJOIN_PREFETCH_DISTANCE:
              cmd_params->starjoin_prefetch_distance = (uint32_t) atoi(optarg);
              break;

          case OPT_STARJOIN_PRO_ORDER:
              cmd_params->starjoin_pro_order = atoi(optarg);
              if(cmd_params->starjoin_pro_order != 0 &&
                 cmd_params->starjoin_pro_order != 1) {
                  printf("[ERROR] --starjoin-pro-order must be 0 or 1.\n");
                  print_help(argv[0]);
                  exit(EXIT_FAILURE);
              }
              break;

...
#include <stdint.h>             /* SIZE_MAX, uint*_t */
...
#include "vector_join.h"        /* vector_join_get_payload_width */
...
static pthread_mutex_t prvj_stats_lock = PTHREAD_MUTEX_INITIALIZER;
static uint64_t prvj_partition_vector_total_bytes = 0;
static uint64_t prvj_partition_count = 0;

static void
prvj_die(const char * msg)
{
    fprintf(stderr, "[ERROR] PRVJ: %s\n", msg);
    exit(EXIT_FAILURE);
}

static void
prvj_die_key(const char * msg, intkey_t key)
{
    fprintf(stderr, "[ERROR] PRVJ: %s key=%lld\n", msg, (long long) key);
    exit(EXIT_FAILURE);
}

static void
prvj_die_payload(const char * msg, value_t payload, uint32_t payload_width)
{
    fprintf(stderr, "[ERROR] PRVJ: %s payload=%lld payload_width=%u\n",
            msg, (long long) payload, payload_width);
    exit(EXIT_FAILURE);
}

static uint64_t
prvj_checked_mul_bytes(uint64_t nitems, uint64_t item_size, const char * msg)
{
...
static uint64_t
prvj_checked_add_bytes(uint64_t lhs, uint64_t rhs, const char * msg)
{
...
static void *
prvj_calloc(uint64_t nitems, uint64_t item_size, uint64_t * requested_bytes,
            const char * msg)
{
    void * ptr;
    uint64_t bytes = prvj_checked_mul_bytes(nitems, item_size, msg);

    ptr = calloc((size_t) nitems, (size_t) item_size);
    if(!ptr) {
        perror("[ERROR] PRVJ allocation failed");
        exit(EXIT_FAILURE);
    }
...
static void
prvj_store_payload(void * payloads, uint32_t payload_width,
                   uint64_t slot, uint64_t payload)
{
    switch(payload_width) {
      ...
      case 4:
          ...
          break;
      default:
          prvj_die("invalid payload width");
    }
}

static value_t
prvj_load_payload(const void * payloads, uint32_t payload_width, uint64_t slot)
{
    switch(payload_width) {
      case 1:
...
static void
prvj_stats_reset(void)
{
    pthread_mutex_lock(&prvj_stats_lock);
    prvj_partition_vector_total_bytes = 0;
    prvj_partition_count = 0;
    pthread_mutex_unlock(&prvj_stats_lock);
}

static void
prvj_stats_add_partition(uint64_t vector_total_bytes)
{
    pthread_mutex_lock(&prvj_stats_lock);
    prvj_partition_vector_total_bytes =
        prvj_checked_add_bytes(prvj_partition_vector_total_bytes,
                               vector_total_bytes,
                               "partition vector byte count overflows");
    prvj_partition_count++;
    pthread_mutex_unlock(&prvj_stats_lock);
}
...
/**
 * Join one radix partition pair by building a vector index for R and probing S.
 */
int64_t
partition_vector_join(const relation_t * const R,
                      const relation_t * const S,
                      relation_t * const tmpR,
                      void * output)
{
    uint64_t i;
    intkey_t max_dense_key = 0;
    uint64_t nslots;
    uint32_t payload_width = vector_join_get_payload_width();
    uint64_t present_bytes = 0;
    uint64_t payload_bytes = 0;
    uint64_t vector_total_bytes;
    uint8_t * present;
    void * payloads;
    int64_t matches = 0;
    ...
    if(R->num_tuples == 0 || S->num_tuples == 0) {
        return 0;
    }

    for(i = 0; i < R->num_tuples; i++) {
        intkey_t key = R->tuples[i].key;
        intkey_t dense_key;

        if(key < 0) {
            prvj_die_key("R key must be non-negative", key);
        }
        dense_key = key >> NUM_RADIX_BITS;
        if(dense_key > max_dense_key) {
            max_dense_key = dense_key;
        }
    }

    nslots = (uint64_t) max_dense_key + 1;
    if(nslots == 0) {
        prvj_die("vector index size overflows addressable memory");
    }

    present = (uint8_t *) prvj_calloc(nslots, sizeof(uint8_t),
                                      &present_bytes,
                                      "present vector size overflows addressable memory");
    payloads = prvj_calloc(nslots, payload_width, &payload_bytes,
                           "payload vector size overflows addressable memory");
    vector_total_bytes = prvj_checked_add_bytes(payload_bytes, present_bytes,
                                                "vector index byte count overflows");

    for(i = 0; i < R->num_tuples; i++) {
        intkey_t key = R->tuples[i].key;
        value_t payload = R->tuples[i].payload;
        uint64_t slot = (uint64_t) (key >> NUM_RADIX_BITS);

        if(present[slot]) {
            prvj_die_key("duplicate R key is not supported", key);
        }
        if(payload < 0) {
            prvj_die_payload("R payload must be non-negative", payload,
                             payload_width);
        }

        /* Width 1/2/4 models compressed payload storage; the join result count
         * is determined by present-key lookup, not the exact payload value.
         */
        prvj_store_payload(payloads, payload_width, slot, (uint64_t) payload);
        present[slot] = 1;
    }

    prvj_stats_add_partition(vector_total_bytes);

    for(i = 0; i < S->num_tuples; i++) {
        intkey_t key = S->tuples[i].key;
        uint64_t slot;
        ...
        slot = (uint64_t) (key >> NUM_RADIX_BITS);
        if(slot >= nslots) {
            continue;
        }

        if(present[slot]) {
            volatile value_t payload =
                prvj_load_payload(payloads, payload_width, slot);
            (void) payload;
            matches++;
        }
    }
...
join_init_run(relation_t * relR, relation_t * relS, JoinFunction jf, int nthreads)
{
    ...
    joinresult = (result_t *) calloc(1, sizeof(result_t));
...
/** \copydoc PRVJ */
result_t *
PRVJ(relation_t * relR, relation_t * relS, int nthreads)
{
    result_t * joinresult;

    prvj_stats_reset();
    joinresult = join_init_run(relR, relS, partition_vector_join, nthreads);

    pthread_mutex_lock(&prvj_stats_lock);
    joinresult->partition_vector_total_bytes =
        prvj_partition_vector_total_bytes;
    joinresult->partition_count = prvj_partition_count;
    pthread_mutex_unlock(&prvj_stats_lock);

    joinresult->has_prvj_stats = 1;
    joinresult->payload_width_bytes = vector_join_get_payload_width();
    joinresult->radix_bits = NUM_RADIX_BITS;
    joinresult->num_passes = NUM_PASSES;

...
try:
    from scripts.benchmark_schema import (
        FLOAT_FIELDS as BASE_FLOAT_FIELDS,
        INT_FIELDS as BASE_INT_FIELDS,
        RAW_FIELDS,
        STATUSES,
        BenchmarkRecord,
    )
...
REQUEST_FIELDS.extend(Formal_REQUEST_FIELDS)
...
REQUEST_INT_FIELDS = BASE_INT_FIELDS | {
    "resident_host_bytes",
    "resident_gpu_bytes",
    "resident_pinned_bytes",
    "request_index",
}
REQUEST_FLOAT_FIELDS = BASE_FLOAT_FIELDS | {
    "dataset_load_ms",
    "session_setup_ms",
    "tune_ms",
    "selected_cpu_ratio",
...
@dataclass(frozen=True)
class FormalBenchmarkRecord(BenchmarkRecord):
    load_ms: float | None
...
def validate_setup(raw: Mapping[str, object]) -> FormalSetupRecord:
    values = _convert(
        raw,
        SETUP_FIELDS,
        SETUP_INT_FIELDS,
    ...
    return FormalSetupRecord(**values)


def validate_request(raw: Mapping[str, object]) -> FormalBenchmarkRecord:
    values = _convert(
        raw,
        REQUEST_FIELDS,
        REQUEST_INT_FIELDS,
    ...
    return FormalBenchmarkRecord(**values)
...
def read_setups(path: Path) -> list[FormalSetupRecord]:
    return _read_records(path, SETUP_FIELDS, validate_setup)


def read_requests(path: Path) -> list[FormalBenchmarkRecord]:
    return _read_records(path, REQUEST_FIELDS, validate_request)


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate MEMQ5 Formal resident records")

...
from scripts.prepare_formal_sf10 import Sf10Paths, main, parse_args, preflight, prepare_sf10, stage_state
...
def test_preflight_rejects_less_than_40_gib(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "scripts.prepare_formal_sf10.shutil.disk_usage",
...
def test_raw_stage_runs_from_dbgen_directory_with_dss_environment_and_provenance(
    ...
    monkeypatch.setattr("scripts.prepare_formal_sf10.subprocess.run", fake_run)
    args = argparse.Namespace(
        dbgen=dbgen,
        raw_dir=paths.raw,
        q5_dir=paths.q5,
...
def test_prepare_records_free_space_from_each_stage_output(
    ...
    monkeypatch.setattr("scripts.prepare_formal_sf10._run_stage", fake_run_stage)
    monkeypatch.setattr("scripts.prepare_formal_sf10.shutil.disk_usage", fake_disk_usage)
    args = argparse.Namespace(
        dbgen=Path("dbgen"),
        raw_dir=paths.raw,
        q5_dir=paths.q5,
...
def test_defaults_follow_the_sf10_raw_q5_arrow_layout(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("sys.argv", ["prepare_formal_sf10.py", "--dbgen", "dbgen"])

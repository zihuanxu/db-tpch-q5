...
from scripts.formal_benchmark_schema import read_requests, read_setups
...
def _write_matrix(
    ...
    *,
    ...
    _write_json(
        ...
        {
            "schema_version": 2,
            "experiment_id": "formal-resident-test",
            "dataset": {
                "path": dataset_path,
                "scale_factor": scale_factor,
...
def test_repository_sf1_formal_matrix_has_exact_required_configurations() -> None:
    from scripts.run_formal_benchmarks import configurations, load_matrix

    matrix = load_matrix(ROOT / "q5" / "experiments" / "formal_sf1.json")

    assert matrix["dataset"]["expected_hash"] == "542abf4003633c7c"
    assert matrix["protocol"] == {"warmup": 3, "repeat": 10, "timeout_seconds": 1800}
    ...
    expected_config_ids = [
        ...
        "gpu-mapped",
        "cudf",
        "hybrid-fixed-r0125-t08",
        "hybrid-fixed-r0250-t08",
        "hybrid-fixed-r0375-t08",
        "hybrid-fixed-r0500-t08",
        "hybrid-fixed-r0625-t08",
        "hybrid-fixed-r0750-t08",
        "hybrid-fixed-r0875-t08",
        "hybrid-auto-t08",
    ]
    assert [config.config_id for config in configurations(matrix)] == expected_config_ids
    assert {config.engine for config in configurations(matrix)} == {
        "cpu-specialized",
        "arrow-acero",
        "gpu-copy",
        "gpu-managed",
    ...
    fixed_ratios = {
        config["config_id"]: config["cpu_ratio"]
        for config in matrix["configurations"]
        if config["ratio_mode"] == "fixed"
    }
    assert fixed_ratios == {
        "cpu-specialized-t08": 1.0,
        "cpu-specialized-t16": 1.0,
        "cpu-specialized-t32": 1.0,
        "arrow-acero-t08": 1.0,
        "arrow-acero-t16": 1.0,
        "arrow-acero-t32": 1.0,
        "gpu-copy": 0.0,
        ...
        "gpu-mapped": 0.0,
        "cudf": 0.0,
        "hybrid-fixed-r0125-t08": 0.125,
        "hybrid-fixed-r0250-t08": 0.25,
        "hybrid-fixed-r0375-t08": 0.375,
        "hybrid-fixed-r0500-t08": 0.5,
        "hybrid-fixed-r0625-t08": 0.625,
        "hybrid-fixed-r0750-t08": 0.75,
        "hybrid-fixed-r0875-t08": 0.875,
    }
    assert {
        config["config_id"]: config["correctness_backend"]
        for config in matrix["configurations"]
        if config["ratio_mode"] == "fixed"
    } == {
        **{
            config_id: "cpu-specialized"
            for config_id in (
                "cpu-specialized-t08",
                "cpu-specialized-t16",
                "cpu-specialized-t32",
        ...
        **{
            config_id: "arrow-acero"
            for config_id in ("arrow-acero-t08", "arrow-acero-t16", "arrow-acero-t32")
        },
        "gpu-copy": "gpu-copy",
    ...
    auto = next(
        config
        for config in matrix["configurations"]
        if config["config_id"] == "hybrid-auto-t08"
    )
    assert auto["ratio_mode"] == "auto"
    assert auto["correctness_backend"] == "hybrid-auto"
    assert auto["mode_options"] == {"selection": "auto"}
...
def test_hybrid_auto_configuration_builds_enableable_session_command(
    ...
    from scripts.run_formal_benchmarks import Configuration, build_command
...
def test_build_commands_consume_current_cpp_and_cudf_session_clis(tmp_path: Path) -> None:
    ...
    cpp_command = build_command(
        cpp,
        session_cli=Path("build-cuda/memq5_arrow_session"),
        dataset=tmp_path,
        region="ASIA",
        date="1994-01-01",
        warmup=1,
    ...
    assert cpp_command[0] == "build-cuda/memq5_arrow_session"
    ...
    assert "q5/baselines/cudf_q5_session.py" in cudf_command


def test_runner_monitors_one_process_and_writes_complete_bundle(tmp_path: Path) -> None:
    from scripts.run_formal_benchmarks import run_matrix
    ...
    assert environment["formal_runner"]["matrix_payload"] == json.loads(
        matrix.read_text(encoding="utf-8")
    )
    assert environment["formal_runner"]["session_cli_sha256"] == _sha256(executable)
    assert environment["formal_runner"]["git_commit"]
    assert environment["formal_runner"]["cudf_env"] == "memq5-cudf"
    assert environment["formal_runner"]["gpu_index"] == 0
    assert environment["formal_runner"]["cuda_visible_devices"] == "0"
...
def test_runner_round_trips_hybrid_auto_setup_provenance_with_gpu_alias() -> None:
    ...
    from scripts.run_formal_benchmarks import Configuration, _setup_from_session
...
def _write_correctness_inputs(
    ...
    *,
    ...
    _write_json(
        ...
        {
            "schema_version": 1,
            "experiment_id": "formal-sf10-correctness",
            "dataset": {
...
def test_materializer_emits_existing_correctness_schema_with_exact_rows(
    ...
    from scripts.run_formal_benchmarks import materialize_correctness_records, run_matrix
    from scripts.formal_correctness_gate import verify_correctness
...
def test_materializer_rejects_tampered_embedded_source_matrix(tmp_path: Path) -> None:
    ...
    environment["formal_runner"]["matrix_payload"]["dataset"]["path"] = "data/other"

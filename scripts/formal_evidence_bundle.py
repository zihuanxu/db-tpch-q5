...
try:
    from scripts.run_formal_benchmarks import (
        Configuration,
        _payload_sha256,
        build_command,
        canonical_command,
    ...
    from scripts.summarize_formal_records import (
        summarize_records,
        summary_fieldnames,
        write_markdown,
        write_summary,
    ...
    from run_formal_benchmarks import (  # type: ignore[no-redef]
        Configuration,
        _payload_sha256,
        build_command,
        canonical_command,
    ...
    from summarize_formal_records import (  # type: ignore[no-redef]
        summarize_records,
        summary_fieldnames,
        write_markdown,
        write_summary,
...
def _validate_sources(
    ...
    if matrix["protocol"]["warmup"] != 3 or matrix["protocol"]["repeat"] != 10:
        raise ValueError("formal evidence requires exactly 3 warmup and 10 measured requests")
    if "oracle" not in matrix:
        raise ValueError("formal matrix requires an independent oracle path and sha256")
...
def _config_matches_setup(config: Configuration, setup: FormalSetupRecord) -> bool:
    if (
...
def _check_log_paths(
    root: Path, setups: Sequence[FormalSetupRecord], requests: Sequence[FormalBenchmarkRecord]
) -> None:
...
def _validate_coverage(
    ...
    setups: Sequence[FormalSetupRecord],
    warmups: Sequence[FormalBenchmarkRecord],
    measured: Sequence[FormalBenchmarkRecord],
) -> None:
    config_by_id = {config.config_id: config for config in configs}
    expected_ids = set(config_by_id)
    setup_by_id: dict[str, FormalSetupRecord] = {}
    for setup in setups:
    ...
    warmups_by_id: dict[str, list[FormalBenchmarkRecord]] = {
        config_id: [] for config_id in expected_ids
    }
    measured_by_id: dict[str, list[FormalBenchmarkRecord]] = {
        config_id: [] for config_id in expected_ids
    }
...
def _validate_environment(
    ...
    runner = environment.get("formal_runner")
...
def _render_summaries(
    measured: Sequence[FormalBenchmarkRecord], setups: Sequence[FormalSetupRecord]
) -> tuple[bytes, bytes, list[dict[str, object]]]:
    rows = summarize_records(measured, setups)
    with tempfile.TemporaryDirectory(prefix="formal-summary-") as directory:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Finalize or audit a Formal evidence bundle")

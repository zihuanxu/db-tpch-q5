...
try:
    ...
    from formal_benchmark_schema import (
        REQUEST_FIELDS,
        REQUEST_MEASUREMENT_FIELDS,
        SETUP_FIELDS,
        SETUP_MEASUREMENT_FIELDS,
...
@dataclass(frozen=True)
class BundleResult:
    setups: tuple[FormalSetupRecord, ...]
    warmups: tuple[FormalBenchmarkRecord, ...]
    measured: tuple[FormalBenchmarkRecord, ...]
    failures: int
...
def _validate_engine_options(
    ...
    *,
    ...
    if engine not in ALL_ENGINES:
        raise ValueError(f"unsupported Formal resident engine: {engine}")
...
def _validate_matrix(matrix: object) -> dict[str, Any]:
    if not isinstance(matrix, dict) or matrix.get("schema_version") != 2:
        raise ValueError("Formal resident matrix schema_version must be 2")
    _strict_fields(
        matrix,
        required={"schema_version", "experiment_id", "dataset", "query", "protocol"},
        optional={"engines", "configurations", "oracle"},
        label="Formal resident matrix",
    )
    if not isinstance(matrix.get("experiment_id"), str) or not matrix["experiment_id"]:
        raise ValueError("Formal resident matrix experiment_id must be nonempty")

    dataset = matrix.get("dataset")
    if not isinstance(dataset, dict):
        raise ValueError("Formal resident matrix dataset must be an object")
    _strict_fields(
        dataset,
        required={"path", "scale_factor", "manifest_sha256", "expected_hash"},
        label="Formal resident matrix dataset",
    )
    for name in ("path", "scale_factor"):
        if not isinstance(dataset.get(name), str) or not dataset[name]:
            raise ValueError(f"Formal resident matrix dataset.{name} must be nonempty")
    if not _is_lower_hex(dataset.get("manifest_sha256"), SHA256_LENGTH):
        raise ValueError("Formal resident matrix requires a lowercase manifest_sha256")
    if not _is_lower_hex(dataset.get("expected_hash"), HASH_LENGTH):
        raise ValueError("Formal resident matrix requires a 16-hex expected_hash")

    oracle = matrix.get("oracle")
    if oracle is not None:
        if not isinstance(oracle, dict):
            raise ValueError("Formal resident matrix oracle must be an object")
        _strict_fields(
            oracle,
            required={"path", "sha256"},
            label="Formal resident matrix oracle",
        )
        if not isinstance(oracle.get("path"), str) or not oracle["path"]:
            raise ValueError("Formal resident matrix oracle.path must be nonempty")
        if not _is_lower_hex(oracle.get("sha256"), SHA256_LENGTH):
            raise ValueError("Formal resident matrix oracle.sha256 must be lowercase SHA256")
    ...
    if isinstance(query, dict):
        _strict_fields(
            ...
            label="Formal resident matrix query",
        )
    if not isinstance(query, dict) or any(
        not isinstance(query.get(name), str) or not query[name]
        for name in ("region", "date")
    ):
        raise ValueError("Formal resident matrix query identity is invalid")

    protocol = matrix.get("protocol")
    if not isinstance(protocol, dict):
        raise ValueError("Formal resident matrix protocol must be an object")
    _strict_fields(
        protocol,
        required={"warmup", "repeat", "timeout_seconds"},
        label="Formal resident matrix protocol",
    )
    warmup = protocol.get("warmup")
    repeat = protocol.get("repeat")
    timeout = protocol.get("timeout_seconds")
    if isinstance(warmup, bool) or not isinstance(warmup, int) or warmup < 0:
        raise ValueError("Formal resident matrix warmup must be nonnegative")
    if isinstance(repeat, bool) or not isinstance(repeat, int) or repeat < 1:
        raise ValueError("Formal resident matrix repeat must be positive")
    if (
        ...
        raise ValueError("Formal resident matrix timeout_seconds must be positive")

    engines = matrix.get("engines")
    explicit = matrix.get("configurations")
    if (engines is None) == (explicit is None):
        raise ValueError("Formal resident matrix requires exactly one of engines or configurations")
    if engines is not None:
        if not isinstance(engines, dict) or not engines:
            raise ValueError("Formal resident matrix engines must be a nonempty object")
        for engine, options in engines.items():
            if not isinstance(options, dict):
                raise ValueError(f"Formal resident engine options for {engine} must be an object")
            _validate_engine_options(
                engine, options, label=f"Formal resident engine {engine}", explicit=False
            )
    else:
        if not isinstance(explicit, list) or not explicit:
            raise ValueError("Formal resident matrix configurations must be a nonempty array")
...
def load_matrix(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"cannot read Formal resident matrix {path}: {exc}") from exc
    ...
    *,
    ...
    if config.runner == "cudf":
        ...
        return [
            ...
            cudf_env,
            "python",
            "q5/baselines/cudf_q5_session.py",
            *common,
        ]
...
) -> tuple[FormalSetupRecord, list[FormalBenchmarkRecord]]:
...
def _prepare_output(output_dir: Path) -> None:
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"Formal output directory is not empty: {output_dir}")
...
def run_matrix(
    *,
    ...
    setups: list[FormalSetupRecord] = []
    requests: list[FormalBenchmarkRecord] = []
...
def _audit_materialization_inputs(
    setups: list[FormalSetupRecord],
    warmups: list[FormalBenchmarkRecord],
    measured: list[FormalBenchmarkRecord],
    *,
    expected_warmup: int,
    expected_repeat: int,
) -> None:
...
def _source_matrix_from_environment(setup_csv: Path) -> dict[str, Any]:
    ...
    try:
        ...
        runner = environment["formal_runner"]
...
def materialize_correctness_records(
    *,
    ...
    by_engine: dict[str, list[FormalSetupRecord]] = {}
    for setup in setups:
...
def _run_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run strict Formal resident benchmark sessions")
    parser.add_argument("--matrix", required=True, type=Path)
    parser.add_argument("--project-root", type=Path, default=Path.cwd())
    parser.add_argument(
        "--session-cli",
        type=Path,
        default=Path("build-cuda/memq5_arrow_session"),
    )
...
def _materialize_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Materialize exact-row Formal correctness-gate run records"
    )

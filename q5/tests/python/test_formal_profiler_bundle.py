...
from scripts import formal_profiler_bundle
from scripts.profile_ncu import collect_ncu
from scripts.profile_nsys import collect_nsys
from scripts.formal_profiler_bundle import artifact_checksums, audit, finalize, sha256_file
...
def _dependency_free_nsys_replay(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(formal_profiler_bundle, "_find_nsys", lambda: executable, raising=False)
    ...
    monkeypatch.setattr(formal_profiler_bundle.subprocess, "run", fake_run)
...
def test_control_io_uses_dirfd_nofollow_static_protection() -> None:
    source = Path(formal_profiler_bundle.__file__).read_text(encoding="utf-8")
...
def test_nsys_reexport_mismatch_fails_when_compatible_tool_is_available(
    ...
    _single_bundle(tmp_path)
    monkeypatch.setattr(formal_profiler_bundle, "_find_nsys", lambda: "/usr/bin/nsys")
...
def test_finalize_requires_nsys_on_the_current_audit_machine(
    ...
    _single_bundle(tmp_path)
    monkeypatch.setattr(formal_profiler_bundle, "_find_nsys", lambda: None)
...
def test_finalize_rejects_an_incompatible_nsys_version(
    ...
    monkeypatch.setattr(formal_profiler_bundle.subprocess, "run", incompatible_version)

...
@pytest.mark.parametrize("entry_point", ["arrow_q5.py", "cudf_q5_session.py"])
def test_baseline_entry_point_help_runs_in_clean_process(entry_point: str) -> None:
    completed = subprocess.run(
        [sys.executable, str(BASELINES_DIR / entry_point), "--help"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,

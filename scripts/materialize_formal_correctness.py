#!/usr/bin/env python3
"""Recompute the formal request gate against an independent Q5 oracle."""
...
try:
    ...
    from scripts.formal_correctness_gate import compare_rows
    from scripts.formal_evidence_bundle import _validate_coverage
    ...
    from formal_benchmark_schema import read_requests, read_setups  # type: ignore[no-redef]
    from formal_correctness_gate import compare_rows  # type: ignore[no-redef]
    from formal_evidence_bundle import _validate_coverage  # type: ignore[no-redef]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Recompute formal request rows against the independent oracle"
    )

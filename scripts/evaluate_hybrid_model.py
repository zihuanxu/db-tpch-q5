...
def _load_environment(directory: Path) -> SourceIdentity:
    ...
    if path is None:
        ...
        raise ValueError(f"cannot find Formal environment identity; searched: {searched}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
        runner = payload["formal_runner"]
        ...
        raise ValueError(f"cannot read Formal environment identity from {path}: {exc}") from exc
...
def _arguments(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate Formal hybrid auto selection against fixed-ratio sweeps"
    )
    parser.add_argument(
        "inputs",
        nargs="+",

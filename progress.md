...
## GitHub Publication Verification (2026-07-14)

- [x] Confirm that raw NSYS reports can retain unrelated inherited process
  environment state and keep only sanitized reports in publication artifacts.
- [x] Add regressions for collection isolation, export sanitization, and
  release-time publication-policy validation.
...
- [x] Regenerate all 10 public NSYS reports with length-stable redaction and
  checksum-bound redaction counts.
...
- [x] Close independent review findings for nested-report policy bypass and
...
Public artifacts contain only the profiler content required to reproduce the
reported observations. Raw local profiler bundles are excluded from the
release because they may contain unrelated process-environment metadata.
